...
#2.10. Г={a,b}. В слове P все символы a заменить на b, а все (прежние) символы b – на a.
...
def change_char_to_num(elem):
    if(elem == '0'): return 0 # для первого состояния
    if(elem == 'a'): return 1 # для второго состояния
    if(elem == 'b'): return 2 # для третьего состояния
...
output = turing_machine({ # сама программа это Python dictionary, в котором каждому состоянию соответствуют два ряда):
    #первое, второе, третье состояния (если 0, если а, если b)
    'A' : (('0', R, 'A'), ('b', R, 'B'), ('a', R, 'B')),
    'B' : (('0', S, 'C'), ('b', R, 'B'), ('a', R, 'B')),
    'C' : (('0', S, 'C'), ('a', S, 'C'), ('b', S, 'C'))},
